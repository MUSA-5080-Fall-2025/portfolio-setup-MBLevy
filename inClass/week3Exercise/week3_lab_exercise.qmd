---
title: "Week 3 In-Class Exercuise: Data Visualization and EDA"
author: "Matthew Levy"
date: 9/22/2025
format: 
  html:
...
census_api_key(Sys.getenv("f6ac8f4f33d6431a4a6a8713369bc824e47dc1e4"))
...
income_vars <- acs_vars_2022 %>%
  ...
  ##[Mm] searches for both Median and median
...
```{r}
medAge <- acs_vars_2022 %>% filter(str_detect(concept, "[mM]edian Age"))
head(medAge)
...
```{r}
#Histogram of total pop of PA coutnies
ggplot(county_data) +
  aes(x = total_popE) +
...
#Boxplot of total pop of PA counties
ggplot(county_data) +
  aes(y = total_popE) +
  geom_boxplot(fill = "purple3", width = 0.5) +
  labs(
...
bottom3 <- county_data %>% arrange(total_popE)
bottom3[1:3,]
top3 <- county_data %>% arrange(desc(total_popE))
top3[1:3,]
```
...
ggplot(county_data) +
  ...
  geom_smooth(method = "loess", se = TRUE, color = "lightblue2") +
  labs(
...
medAgeMoE <- county_data %>% mutate(
  ageMoEPct = (median_ageM/median_ageE)*100,
   pop_category = case_when(
      total_popE < 50000 ~ "Small (<50K)",
      total_popE < 200000 ~ "Medium (50K-200K)",
      TRUE ~ "Large (200K+)"
...
ggplot(medAgeMoE) +
  aes(x = total_popE , y = ageMoEPct) +
  geom_point(alpha = 0.7, color = "purple3") +
  geom_hline(yintercept = 10, color = "lightblue2", linetype = "dashed") +
  labs(
    title = "Data Reliability Decreases with Population Size",
    x = "Total Population",
    y = "Margin of Error (%)",
...
# Box plots by population category
ggplot(medAgeMoE) +
  aes(x = pop_category, y = ageMoEPct, fill = pop_category) +
  geom_boxplot() +
  labs(
    title = "Data Reliability by County Size Category",
    x = "Population Category",
...
```{r}
incomeFaceted <- county_data %>%
  mutate(
    income_category = case_when(
      median_incomeE < 50000 ~ "Low (< $50k)",
      median_incomeE <= 80000 ~ "Middle (50-80)",
      TRUE ~ "High (>$80k)"
    )
  )

ggplot(incomeFaceted) +
  aes(x = total_popE, y = median_ageE, color = income_category, size = median_incomeM) +
  geom_point(alpha = .5) +
  scale_color_brewer(palette = "Set2") +
  theme_bw() +
  facet_wrap(~income_category) +
  labs(
    title = "Population-Age Relationship by Income Profile",
    x = "Total population",
    y = "Median Income ($)"
